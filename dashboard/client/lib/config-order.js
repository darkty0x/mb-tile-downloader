...
export function reorderConfigChoice(items = [], fromIndex, toIndex, { grouped = false, position = "before" } = {}) {
  ...
  const targetGroupKey = grouped ? groupKeyForConfigChoice(target) : "";
  const insertPosition = position === "after" ? "after" : "before";
  const targetIndexes = remaining
    .map((item, itemIndex) => (
      grouped && groupKeyForConfigChoice(item) === targetGroupKey
        ? itemIndex
    ...
    .filter((itemIndex) => itemIndex >= 0);
  if (!targetIndexes.length) return items;
  const insertAt = insertPosition === "after" ? Math.max(...targetIndexes) + 1 : Math.min(...targetIndexes);
  return [
    ...remaining.slice(0, insertAt),
    ...block,
    ...remaining.slice(insertAt),
