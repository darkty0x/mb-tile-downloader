...
export function Rail({ state, actions }) {
  ...
  const navCount = (tab) => {
    ...
    if (tab === "credentials") return state.secretPool.filter((secret) => ["credential", "server_rdp_credential"].includes(secret.secretType)).length;
      ...
      <nav className="mt-6 grid gap-2 max-md:mt-0 max-md:flex max-md:min-w-max max-md:gap-2" aria-label="관리체계 구역">
        {TABS.map(([tab, label, icon]) => {
          ...
          return (
            ...
            >
              <Icon name={icon} className={`h-5 w-5 shrink-0 ${state.selectedTab === tab ? "text-[var(--ptg-primary)]" : ""}`} />
              <span className="min-w-0 flex-1 whitespace-nowrap">{label}</span>
              {count === null ? null : <strong className={`grid h-6 w-6 shrink-0 place-items-center rounded-full px-1 text-[11px] leading-none ${state.selectedTab === tab ? "bg-white text-[var(--ptg-primary)]" : "bg-[rgba(255,255,255,0.11)] text-[#eaf1ff]"}`}>{count > 99 ? "99+" : count}</strong>}
            </button>
          );
        })}
      </nav>
