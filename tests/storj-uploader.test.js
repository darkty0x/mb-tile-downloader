...
test("storj uploader explains unauthorized Storj API-key credentials", async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), "storj-uploader-"));
  const archivesDir = path.join(dir, "archives");
  const toolsDir = path.join(path.resolve("."), "tools", "uplink");
  const archivePath = path.join(archivesDir, "tiles_vector_1_000000-000000_y000000-000000.zip");
  ...
  try {
    ...
    await writeFile(
      ...
      [
        "#!/usr/bin/env node",
        "const args = process.argv.slice(2);",
        "if (args.includes('create')) { console.error('uplink: permission denied (Unauthorized API credentials)'); process.exit(1); }",
        "process.exit(0);",
      ].join("\n"),
      { mode: 0o755 }
    );
    ...
    await assert.rejects(
      () =>
        execFileAsync(
          ...
          [
            ...
            `--archive-dir=${archivesDir}`,
            "--bucket=mapbox",
            "--access=121Test@ap1.storj.io:7777 1BadApiKey",
          ],
          {
            cwd: path.resolve("."),
            env: {
      ...
      (err) => {
        assert.match(err.stderr, /Storj API key was rejected by the satellite as unauthorized/);
        assert.match(err.stderr, /Create a new Storj API key in the same project/);
        assert.match(err.stderr, /Alternatively set STORJ_ACCESS to one serialized Access Grant value/);
        return true;
      }
    );
  } finally {
